...
class IPStream(object):
    # name=None, ip_address=None, username=None, passwd=None,
    # vendor="vivotek", protocol="rtsp", rotation=0, roi=None,
    # detect_region=None, fps=3, resolution="1024x768"

    def __init__(self, params):

        self.camera_name = params['name'] if params[
            'name'] is not None else random.random()
        ...
        self.sensitivity = float(params['sensitivity']) if params[
            'sensitivity'] is not None else 10
        self.analize_frame = AnalizeFrame.AnalizeFrame()
        self.analize_plate = AnalizePlate.AnalizePlate()
        self.helper = Helper.Helper()

        self.url = self.helper.generate_url(
            self.ip_address,
            params['username'],
            params['passwd'],
            self.vendor,
    ...
    def proccess(self, stream):
        ...
        if self.detect_region:
            dht = int(
                self.detect_region[0]) if self.detect_region is not -1 else 0
            dhb = int(self.detect_region[
                1]) if self.detect_region is not -1 else jpg.shape[0]
            dwt = int(
                self.detect_region[2]) if self.detect_region is not -1 else 0
            dwb = int(self.detect_region[
                3]) if self.detect_region is not -1 else jpg.shape[1]
        # print dht,dhb,dwt,dwb
        # print "Konektovan na ",self.ip_address
        while ret:
            try:
                if (isinstance(jpg, type(None))):
                    break
                if self.rotation:
                ...
                try:
                    if self.roi:
                        his = self.analize_frame.calcHist(
                            frame[ht:hb, wt:wb], True)
                    else:
                ...
                if his > self.sensitivity and now > pause:
                    ...
                    if self.detect_region:
                        thread.start_new_thread(
                            self.tst, (jpg[dht:dhb, dwt:dwb], jpg,))
                    else:
